...
class ImuProcess
{
 public:
  ...
  Eigen::Matrix<double, 12, 12> predict_Q;
  void Process(const MeasureGroup &meas,  esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI::Ptr pcl_un_);
  void ImuPredict(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state);
 ...
 private:
  ...
  sensor_msgs::ImuConstPtr last_predict_imu_;
  deque<sensor_msgs::ImuConstPtr> v_imu_;
  vector<Pose6D> IMUpose;
  vector<Pose6D> predict_IMUpose;
    ...
    : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1)
{
  ...
  last_predict_imu_.reset(new sensor_msgs::Imu());
}
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  last_predict_imu_ = meas.imu.back();
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  const double &pcl_end_time = meas.lidar_end_time_;                // pcl结束的时间戳
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    // fout_imu << setw(10) << head->header.stamp.toSec() - first_lidar_time_ << " " << angvel_avr.transpose() << " " << acc_avr.transpose() << endl;
    //通过重力数值对加速度进行一下微调
  ...
  auto pcl_cur_begin = pcl_out.points.begin()->curvature;
...
void ImuProcess::ImuPredict(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state)
{
  if(meas.imu.empty() || imu_need_init_) {return;};

  /*** add the imu of the last frame-tail to the of current frame-head ***/
  auto v_imu = meas.imu;                                            //拿到当前的imu数据
  v_imu.push_front(last_predict_imu_);                                      //将上一帧最后尾部的imu添加到当前帧头部的imu
  const double &imu_beg_time = v_imu.front()->header.stamp.toSec(); //拿到当前帧头部的imu的时间（也就是上一帧尾部的imu时间戳）
  const double &imu_end_time = v_imu.back()->header.stamp.toSec();  //拿到当前帧尾部的imu的时间
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    in.acc = acc_avr;
    in.gyro = angvel_avr;
    predict_Q.block<3, 3>(0, 0).diagonal() = cov_gyr;
    predict_Q.block<3, 3>(3, 3).diagonal() = cov_acc;
    predict_Q.block<3, 3>(6, 6).diagonal() = cov_bias_gyr;
    predict_Q.block<3, 3>(9, 9).diagonal() = cov_bias_acc;
    kf_state.predict(dt, predict_Q, in);
  }
