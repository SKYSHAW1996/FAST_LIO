...
#define Multi_Thread_Rebuild_Point_Num 1500 //  需要重置的subtree点数达到此阈值，则用额外线程处理。
#define DOWNSAMPLE_SWITCH true              //  新增点时，是否启动ikdtree内的downsample
#define ForceRebuildPercentage 0.2          //  
#define Q_LEN 1000000                       //  支持的最大队列长度
...
class KD_TREE {
  ...
  private:
    ...
    KD_TREE_NODE * STATIC_ROOT_NODE = nullptr; //  `根节点`的`根`，tree结构依存的基础，仅在build或彻底rebuild时改变之。
  ...
  public:
    ...
    void Nearest_Search(PointType point, int k_nearest, PointVector &nearest_points_, 
