...
void KD_TREE<PointType>::multi_thread_rebuild(){
    ...
    // KD_TREE_NODE ** new_node_ptr; // : unused variable.
    pthread_mutex_lock(&termination_flag_mutex_lock);
    ...
    while (!terminated){
        ...
        if (Rebuild_Ptr != nullptr ){                    
            ...
            // int valid_old = old_root_node->TreeSize-old_root_node->invalid_point_num; // : unused variable.
            // int valid_new = new_root_node->TreeSize-new_root_node->invalid_point_num; // : unused variable.
...
void KD_TREE<PointType>::Build(PointVector point_cloud){
    ...
    STATIC_ROOT_NODE = new KD_TREE_NODE; //  内存依赖上的根节点。
    ...
    Root_Node = STATIC_ROOT_NODE->left_son_ptr; //  逻辑上的根节点。
}

//  关键入口函数
template <typename PointType>
void KD_TREE<PointType>::Nearest_Search(
    PointType point, int k_nearest, PointVector& nearest_points_, 
    vector<float> & Point_Distance, double max_dist)
{
    MANUAL_HEAP q(2*k_nearest); //  预分配两倍空间。
    q.clear();
    ...
    PointVector ().swap(nearest_points_);
    vector<float> ().swap(Point_Distance);
    for (int i=0;i < k_found;i++){
        nearest_points_.insert(nearest_points_.begin(), q.top().point);
...
{
    // int NewPointSize = PointToAdd.size();   // : unused variable.
    // int tree_size = size();                 // : unused variable.
...
void KD_TREE<PointType>::BuildTree(KD_TREE_NODE ** root, int l, int r, PointVector & Storage){
    ...
    *root = new KD_TREE_NODE;
    ...
    //  以下，应该是按照主轴方向排序，排序结果放在Storage变量中。
    switch (div_axis)
    {
    case 0:
        //  用C++算法库的函数进行排序，只需确保在mid位置的数大于左侧，且小于右侧即可，不必严格完全排序。
...
void KD_TREE<PointType>::Rebuild(KD_TREE_NODE ** root){
    ...
    if ((*root)->TreeSize >= Multi_Thread_Rebuild_Point_Num) { 
        ...
        // int size_rec = (*root)->TreeSize; // : unused variable.
        PCL_Storage.clear();
                                        ...
                                        bool allow_rebuild, bool is_downsample)
{
    ...
    // struct timespec Timeout; // : unused variable.
...
{     
    //  如果已经到达叶子节点，直接插入，然后返回 —— 在递归工作流中意味着触底。
    if (*root == nullptr){
        *root = new KD_TREE_NODE;
        InitTreeNode(*root);
        (*root)->point = point;
...
void KD_TREE<PointType>::Search(KD_TREE_NODE * root, int k_nearest, PointType point, MANUAL_HEAP &q, double max_dist)
{
    ...
    //  如同论文中讲到的，搜索到任一个节点时，首先根据节点的range信息，比较节点张成的空间是否与point距离张成的球空间有交叉？
    //  如果无交叉，则不可能最在ranged-kNN解，直接退出（剪枝加速）。
    ...
    //  继续向下递归搜索。「逻辑核心*」
    // int cur_search_counter; // : unused variable.
...
//  工具属性，更新指定节点的min/max-range, treesize, radius_sq, alpha_del等信息。
template <typename PointType>
...
float KD_TREE<PointType>::calc_box_dist(KD_TREE_NODE * node, PointType point){
    ...
    float min_dist = 0.0;
    //  因为算出来的distance仅用于比较，因此只需算出distance的平方就行，无需开方。
